...
class Funatlas:
    ...
    def get_deltaFoverF(self, occ2, interval, normalize=["baseline"],
                        return_all=False,exclude_ds=[]):
        ...
        occ2:
            Reference occ2 from get_occurrence_matrix.
        interval: array_like (2,)
            Time interval. Zero at stimulation time.
        ...
        for ineu in np.arange(n_neu):
            for jneu in np.arange(n_neu):
                ...
                if return_all: dFF_all[ineu,jneu] = dff
    ...
    def weighted_avg_occ2style2(A, ws, return_rele=False, abs_w=True):
        ...
        for i in np.arange(n):
            for j in np.arange(m):
                ...
                if len(A[i,j])>0 and np.nansum(w)>0:
    ...
    @staticmethod
    def get_significance_features(sig,neu_i,i0,i1,shift_vol,Dt,nan_th,sderker=None):
        if sderker is None:
        ...
        if np.sum(nan_mask[:,neu_i])>nan_th*len(y[:,neu_i]):
            ...
            if pre==0.0:
                ...
                dy = np.average( y[shift_vol:shift_vol+60,neu_i] - baseline )/pre #This was not bound to :+60
                ...
                for ie in np.arange(fconn.n_stim):
                    ...
                    for ineu in np.arange(fconn.n_neurons):
                        ...
                        '''if np.sum(nan_mask[:,ineu])>nan_th*len(y[:,ineu]):
                            ...
                            baseline = np.average(y[:shift_vol,ineu])
                            #pre = baseline
                            pre = sig.get_loc_std(y[:shift_vol,ineu],8)
                            if pre==0.0:
                                continue
                            else:
                                dy = np.average( y[shift_vol:,ineu] - baseline )/pre
                                sd__ = np.sum(sder[i0+shift_vol-5:i0+shift_vol+11,ineu]) #+6
                                dff_[ie,ineu] = dy
                                dff2_[ie,ineu] = dy
                                sd_[ie,ineu] = sd__'''
    ...
    def get_kolmogorov_smirnov_p(self,inclall_occ2,nan_th=0.3,**kwargs):
        ctrl_dff, ctrl_dff2, ctrl_sd = self.get_ctrl_distributions(**kwargs)
        
        n_neu = inclall_occ2.shape[0]
        
        '''# Prepare the second derivatives
        n_ds = len(self.ds_list)
        sder = np.empty(n_ds,dtype=object)
        for a in sder: a = []
        for i_ds in np.arange(n_ds):
            sderker = savgol_coeffs(13, 2, deriv=2, delta=self.fconn[i_ds].Dt)
            sder_ = np.zeros_like(self.sig[i_ds].data)            
            for k in np.arange(self.sig[i_ds].data.shape[1]):
                sder_[:,k] = np.convolve(sderker,self.sig[i_ds].data[:,k],
                                         mode="same")
            sder[i_ds] = sder_'''
            ...
            for ai in np.arange(n_neu):
                ...
                for occ in inclall_occ2[ai,aj]:
                    ...
                    if dff__ is not None:
                        ...
                        continue
                    '''y = self.sig[ds].get_segment(i0,i1,baseline=False,normalize="none")
                    nan_mask = self.sig[ds].get_segment_nan_mask(i0,i1)
                    
                    if np.sum(nan_mask[:,i])>nan_th*len(y[:,i]):
                        continue
                    else:
                        baseline = np.average(y[:shift_vol,i])
                        #pre = baseline
                        pre = self.sig[ds].get_loc_std(y[:shift_vol,i],8)
                        if pre==0.0:
                            continue
                        else:
                            dy = np.average( y[shift_vol:,i] - baseline )/pre          
                        sd__ = np.sum(sder[ds][i0+shift_vol-5:i0+shift_vol+11,i]) #+6'''
