...
intermediate_resp = []
fast_resp = []
avg_rise_times = funa.weighted_avg_occ2style(rise_times,dFF)
for respi in np.arange(len(occ2)):
    avg_rise_time = avg_rise_times[respi,ai_RID]
    if not np.isnan(avg_rise_time): 
        if avg_rise_time<10: 
            fast_resp.append(funa.neuron_ids[respi])
        elif 10<=avg_rise_time<20: 
            intermediate_resp.append(funa.neuron_ids[respi])
        elif avg_rise_time>=20:
            slow_resp.append(funa.neuron_ids[respi])
            
print("slow",slow_resp)
print("intermediate",intermediate_resp)
print("fast",fast_resp)
